...
function generate_coverage(pkg::Module; genhtml = true, runtests = true)
    ...
    cd(joinpath(Pkg.pathof(pkg), "..", "..")) do
        generate_coverage(genhtml=genhtml)
    end
end
...
Generate a coverage report for the current direcctory. Assumes that coverage has already been generated by running `Pkg.test(;coverage=true)`
"""
function generate_coverage(;genhtml=true)
