...
"""
function generate_coverage(pkg::Module; genhtml = true, runtests = true)
    if runtests
        Pkg.test(pkg; coverage = true)
    end
    generate_coverage(joinpath(Pkg.pathof(pkg), ".."), genhtml=genhtml)
end

"""
$(SIGNATURES)

Generate a coverage report for the direcctory `path`. Assumes that coverage has already been generated by running `Pkg.test(;coverage=true)`
"""
function generate_coverage(path::String;genhtml=true)
    cd(path) do
